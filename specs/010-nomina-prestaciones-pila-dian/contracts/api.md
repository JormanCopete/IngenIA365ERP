...
**Un solo pagador del último tramo (D-29)**: la definitiva paga `SALARIO_PENDIENTE` (con auxilio) por
...
| `Payroll.Settlement.ConfirmationRequired` | 422 | falta `confirm`, o `confirmEmpty` en una corrida sin empleados | |
| `Payroll.Settlement.SegregationConfirmationRequired` | 422 | falta `confirmWithoutSegregation`: la política lo admite pero exige confirmarlo | |
...
| `POST /{generationPublicId}/download-link?acknowledgeDifference=` | Pila.View | feature 011 (§9 de `specs/011-adjuntos-s3-prefirmadas/contracts/api.md`): `{ direct, url, expiresAt }`, un enlace firmado de 60 s que obliga a guardar el `.txt` con su nombre y `text/plain; charset=` del layout; las mismas reglas que `/file` (`Payroll.Pila.Unreconciled` sin reconocer el descuadre, `Payroll.Pila.FileTampered` si la huella guardada no es la de la generación). Con `direct: false` la planilla es del formato anterior y se baja por `/file`, que desde entonces responde 409 `Attachments.UseDownloadLink` a las del formato directo. Ambas rutas llevan el limitador `adjuntos` (429 `Attachments.Busy`) |
...
| `POST /{id}/download-link` | Disbursement.View | feature 011 (§9): `{ direct, url, expiresAt }`, un enlace firmado de 60 s con el nombre, el `Content-Type` y el `charset` del formato; `Payroll.Disbursement.FileNotFound`, `.FileMissing`, `.FileTampered` como `/file`. Con `direct: false` el archivo es del formato anterior y se baja por `/file`, que responde 409 `Attachments.UseDownloadLink` a los del formato directo. Ambas rutas llevan el limitador `adjuntos` |
