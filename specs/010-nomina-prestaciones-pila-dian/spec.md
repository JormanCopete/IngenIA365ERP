...
según el NIT, el documento de nómina electrónica se transmite en los primeros diez días hábiles
del mes siguiente, y un retiro exige la liquidación al terminar el contrato. Todo esto la
cooperativa lo hace hoy a mano o en otros programas.
...
- Q: ¿Cómo firma y transmite la cooperativa la nómina electrónica a la DIAN? → A: **Por un
  servicio centralizado de Ingenia365** que actúa como proveedor tecnológico para varios clientes:
  cada cooperativa se habilita ante la DIAN como empleador obligado y transmite con el software de
  Ingenia365; el servicio central guarda la habilitación de cada una (NIT, rangos, set de pruebas,
  ambiente), **firma con el certificado que Ingenia365 registra como proveedor** y transmite al
  servicio de recepción de la DIAN; el ERP construye el documento, lo entrega al servicio y recibe
  estado y CUNE. El trámite de registro de Ingenia365 como proveedor tecnológico y la habilitación
  del software ante la DIAN son del dueño y quedan fuera del programa.
...
- Conexión directa con los bancos, con el operador de PILA o con el fondo de cesantías: los
  archivos se generan y la persona los carga; sólo la DIAN (o el proveedor tecnológico) recibe
  transmisión directa.
...
- **Procedimiento 2**: método de retención en la fuente por salarios con un porcentaje fijo que se
  calcula cada semestre con los ingresos de los doce meses anteriores (art. 386 ET).
...
3. **Given** un empleado con salario integral o un aprendiz, **When** liquida, **Then** no aparece
   y la lista dice por qué.
...
identificación del software, ambiente (habilitación o producción). La firma y la transmisión las hace el **servicio centralizado de nómina electrónica de Ingenia365**, que atiende a varias cooperativas con una habilitación por cada una.

**Why this priority**: obligación mensual (Resolución 000013 de 2021) con sanción por no
transmitir; la cooperativa debe cumplirla desde el primer mes de nómina en el sistema.

**Independent Test**: un mes con dos empleados (uno con horas extras y una deducción de
libranza); generar los documentos, validarlos contra el anexo técnico vigente en el ambiente de
pruebas de la DIAN o del proveedor, transmitir, recibir CUNE; reversar una nómina y generar la
...
la nómina ordinaria aplica, obtiene el ingreso mensual promedio en UVT, busca la retención
teórica en la tabla y calcula el porcentaje; lo muestra con su explicación mes a mes y, al
...
2. **Given** menos de doce meses, **Then** promedia los meses trabajados y lo dice.
...
- Aprendiz o pasante: sin prestaciones ni aportes a pensión/caja; PILA con tipo de cotizante
  propio; salud y riesgos según etapa (parámetro).
...
- La DIAN o el proveedor no responden: el documento queda «en proceso» con reintento manual;
...
- **FR-009**: MUST excluir a quien no tiene derecho (salario integral, aprendices, pasantes) y
  descontar la prima proporcional ya pagada en una liquidación definitiva del mismo semestre.
...
- **FR-012**: El sistema MUST producir la relación de consignación por fondo de cesantías
  ...
  cada fondo.
- **FR-013**: Quien tiene salario integral MUST quedar fuera de cesantías e intereses, y quien se
  retiró con definitiva en el año MUST quedar fuera de la anual.
...
- **FR-022**: El sistema MUST calcular, para los empleados marcados en procedimiento 2, el
  ...
  depurados con las deducciones y rentas exentas parametrizadas, promedio mensual en UVT,
  retención teórica con la tabla vigente y porcentaje resultante, con explicación mes a mes.
...
- **FR-031**: La cooperativa MUST parametrizar su habilitación (NIT, rangos de numeración,
  identificación del software, ambiente de habilitación o producción) y el sistema MUST rechazar
  transmitir con la parametrización incompleta.
- **FR-031a**: La firma y la transmisión MUST hacerlas un **servicio centralizado de Ingenia365**
  (un solo servicio para todas las cooperativas) que guarda la habilitación de cada una aislada de
  las demás, firma con el certificado de Ingenia365 como proveedor tecnológico, transmite al
  servicio de recepción de la DIAN, conserva cada envío con su respuesta y devuelve al ERP el
  estado y el CUNE; ninguna cooperativa MUST poder ver ni afectar los documentos de otra, y el
  certificado y las credenciales MUST vivir sólo en ese servicio, nunca en el ERP ni en el
  repositorio.
- **FR-031b**: El servicio central MUST operar contra el ambiente de habilitación de la DIAN para
  el set de pruebas de cada cooperativa y contra producción una vez habilitada, y el ERP MUST
  mostrar en cuál está cada una.
...
- **Documento de nómina electrónica**: empleado, mes, tipo (soporte, nota de ajuste de reemplazo
  o eliminación), número, contenido, estado (generado, transmitido, aceptado, rechazado, en
  proceso), CUNE, respuesta, intentos; **Habilitación**: NIT, rangos, software, medio de firma y
  transmisión.
...
- **Norma laboral colombiana general** como regla por defecto, con todo valor como parámetro con
  ...
  salario integral sin prima ni cesantías; aprendices sin prestaciones. Cualquier pacto distinto
  de la cooperativa se carga como parámetro.
- **Procedimiento 2** según el art. 386 del Estatuto Tributario: ingreso mensual promedio de los
  doce meses anteriores al mes del cálculo, depurado con las mismas reglas que la nómina ordinaria
  usa para el procedimiento 1, en UVT del año, retención teórica con la tabla y porcentaje fijo
  que rige el semestre siguiente.
- **PILA** según el formato de la Resolución 2388 de 2016 y sus modificaciones vigentes, planilla
  ...
  (cooperativa exonerada o no, umbral de salarios mínimos).
- **Nómina electrónica** según la Resolución 000013 de 2021 y el anexo técnico vigente (documento
  mensual por empleado, notas de ajuste, CUNE), firmada y transmitida por el servicio centralizado
  de Ingenia365 como proveedor tecnológico; el registro de Ingenia365 ante la DIAN, el certificado
  de firma y la habilitación del software son trámites del dueño y condicionan cuándo se puede
  transmitir en producción, no lo que el programa hace.
...
- Para la nómina electrónica: el registro de Ingenia365 como proveedor tecnológico, su certificado
  de firma, la habilitación del software y la de cada cooperativa ante la DIAN, y el acceso al
  ambiente de pruebas.
