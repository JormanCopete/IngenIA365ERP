...
| D-09 | Umbral de exoneración | Parámetro legal `EXONERACION_114_1_UMBRAL_SMMLV` = 10 + política por empresa `Exonerada114_1` sí/no con vigencia. |
...
| D-16 | Modo PT declarado por el servicio | Sí: `GET /v1/capabilities` en el contrato; la habilitación de la cooperativa sólo puede elegir `ProveedorTecnologico` si el servicio lo anuncia. |
...
| — | Permisos | Los de `contracts/api.md`: `Payroll.ServiceBonus`, `Payroll.Severance`, `Payroll.Vacations`, `Payroll.Settlements` (definitiva y descuentos), `Payroll.OpeningBalances`, `Payroll.TerminationReasons`, `Payroll.WithholdingRate`, `Payroll.Pila`, `Payroll.ElectronicPayroll`, `Payroll.Disbursement`, `Payroll.BankFileFormats`, `Payroll.Holidays`, `Payroll.Policies`; aprobar/reversar/transmitir/marcar enviado/ajustar descuentos sólo `CompanyAdmin`; `Operator` ve, registra, calcula y genera; `Auditor`/`ReadOnly` `*.View`. |
