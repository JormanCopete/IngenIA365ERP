...
| `DELETE /drafts/{id}?deleteAttachments=` | Vouchers.Create | descarta (soft). Con soportes y sin `deleteAttachments=true`: 422 `Accounting.Document.HasAttachments`, `data: { count }`, y no toca nada; con la marca, los soportes van a la papelera de 90 días y se dan de baja con el borrador (feature 011, R10; vale también para `AP`) |
