...
- [x] T056 [US1] Actualizar `E2E/Payroll/NominaE2E.cs` (`PrepararAsync`: `POST /api/accounting/setup/initialize` con Solidario y nivel 6, creación de auxiliares de movimiento con reglas para nómina, períodos ya no se crean a mano) y crear `E2E/Accounting/ContabilidadNiifTests.cs` con la parte de US1 (inicializar; segundo intento 422; bloqueo tras auxiliar; importar catálogo con error de fila → 422 con `row`; importar un catálogo generado de 2 000 filas termina en < 1 min, SC-001) — **Hecha 2026-09-21** (`IntegrationTests/Accounting/ContabilidadNiifTests.cs`: segunda inicialización 422, `Setup.Locked` con auxiliares, catálogo con fila mala 422 con `row`/`column` y nada guardado, 2.349 filas en < 1 min; `NominaE2E` ya inicializaba desde E1)
...
- [x] T066 [US2] Ampliar `E2E/Accounting/ContabilidadNiifTests.cs` con US2 (auxiliar sólo Nómina: parametrizar en nómina OK; buscador con `module=Lending` no la devuelve; tras contabilizar un comprobante, `PUT` de reglas → 422 `Accounting.Account.Locked`; `DELETE` referenciada → 422 con `references`) — **Hecha 2026-09-21** (`ContabilidadNiifTests`: auxiliar sólo NOM se parametriza en SALARIO, `search?module=CAR` no la devuelve, `DELETE` referenciada → 422 `Referenced` con `references`, `PUT` reglas de cuenta con movimientos → 422 `Locked`, sólo nombre sí)
...
- [x] T077 [US3] Ampliar `E2E/Accounting/ContabilidadNiifTests.cs` con US3 (`/validate` marca tercero faltante; borrador descuadrado se guarda y no se contabiliza; dos clientes contabilizan en paralelo → números consecutivos distintos; cuatro ojos rechaza al mismo usuario; reversar → neto cero; reversar dos veces → 422; cerrar mes con borrador → 422 con lista; tipo `NM` en digitación manual → 422) — **Hecha 2026-09-21** (`ContabilidadNiifTests`: `/validate` señala línea y campo `Person`; descuadrado 201 y post 422; NM a mano 422 `ManualOnly`; cerrar mes con borrador 422 con `drafts`; dos clientes contabilizan en paralelo → números consecutivos distintos; cuatro ojos 422 (se apaga al terminar); reversar → `Reversed` + espejo, dos veces 422, la reversión 422 `IsReversal`)
...
- [x] T084 [US4] Crear `E2E/Payroll/ContabilidadDeNominaTests.cs` (aprobar → `NM` contabilizado con EPS como tercero; `GET /documents/by-source?module=NOM`; en Contabilidad `POST /{id}/reverse` → 422 `ModuleOwned`; concepto a agrupación → aprobación 422 y cero huérfanos en ambos sentidos; reversar desde Nómina → neto cero; nómina con fecha en mes cerrado → 422 `Accounting.Period.Closed`) — **Hecha 2026-09-21** (`IntegrationTests/Payroll/ContabilidadDeNominaTests.cs`, colección Nómina, junio y julio de 2027: NM Posted por `by-source` con el empleado como tercero; reversar desde Contabilidad 422 `ModuleOwned` con `data.origin`; agrupación en concepto → 422 `NotEligible` en la parametrización (cero huérfanos); reversar desde Nómina → espejo cuenta a cuenta; mes cerrado → 422 `Accounting.Period.Closed` y nada en el libro)
...
- [x] T091 [US7] Ampliar `E2E/Accounting/ContabilidadNiifTests.cs` con US7 (`POST /api/audit/access` y el evento `RegisterOptionAccess` aparece en `/api/audit/logs?Module=Navigation` en < 30 s; sólo lectura: toda escritura → 404 `Generic.NotFound` igual que `POST /api/accounting/no-existe`, lecturas 200; Operador: `POST /drafts` 201 y `POST /post` 404; cambio de regla de cuenta con `oldValuesJson`/`newValuesJson`) — **Hecha 2026-09-21** (`ContabilidadNiifTests`: `POST /api/audit/access` → evento `RegisterOptionAccess` en `?module=Navigation` en < 30 s; sólo lectura: escrituras 404 `Generic.NotFound` igual que `/api/accounting/no-existe`, lecturas 200; Operador 201 en `/drafts` y 404 en `/post`; `Accounting.Account.Updated` con `oldValuesJson`/`newValuesJson`)
...
> **2026-09-20**: la rama real fue `009-e2-consultas-presupuesto`: entregó **US5 completa** (T097–T108;
> de T108 falta sólo `RendimientoDeInformesTests`) y adelantó **US9 presupuesto** (T137–T141) de E4, a pedido
> del dueño («no veo el módulo de presupuesto»). US6 (cierre de ejercicio, T109–T112) y US13 (apertura
> importada, T113–T117) siguen pendientes. Desvíos respecto del diseño original: el libro auxiliar
> navega por migas de pan (no `SfTreeGrid`); la consulta consolidada del tercero tiene pantalla propia
> `/contabilidad/terceros`; el Centro de Reportes enlaza a las pantallas del módulo.
...
- [x] T097 [P] [US5] Crear `FiltrosDeInforme` (rango, cuenta/rama, tercero, documento, centro, sucursal, tipo, origen, usuario, nivel, con terceros, incluir cierre, formato) y `EncabezadoDeInforme` (empresa, NIT, filtros, período, usuario, fecha → `Notas` de `TablaExportable`) en `Application/Accounting/Reports/FiltrosDeInforme.cs`
- [x] T098 [P] [US5] Crear `LedgerQuery` (nodos por demanda: class → group → account → subaccount → auxiliary → person → document → voucher → line, cada uno con saldo inicial, débitos, créditos y final; respeta `IUserBranchScope`) en `Application/Accounting/Reports/LedgerQuery.cs`
- [x] T099 [P] [US5] Crear `TrialBalanceQuery` (nivel, con/sin terceros, con/sin cierre; consolidación en memoria; invariante débitos = créditos), `JournalQuery`, `GeneralLedgerQuery`, `VoucherListQuery` en `Application/Accounting/Reports/LibrosQueries.cs`
- [x] T100 [P] [US5] Crear `ThirdPartyStatementQuery` (movimientos por cuenta y documentos con saldo), `PendingDocumentsQuery` (saldo por tercero y documento cruce), `DailyAverageBalanceQuery` (saldo diario desde movimientos, promedio del período) en `Application/Accounting/Reports/TercerosQueries.cs`
- [x] T101 [P] [US5] Crear `FinancialPositionQuery`, `IncomeStatementQuery` (`IncluirCierre`), `EquityChangesQuery`, `CashFlowQuery` (indirecto) sobre `ACC_FinancialStatementItems` con comparativo del período anterior en `Application/Accounting/Reports/EstadosFinancierosQueries.cs`
- [x] T102 [US5] En cada handler de informe de `Application/Accounting/Reports/*Queries.cs` (T098–T101) emitir `Accounting.Report.Exported` (informe, filtros, formato) vía `AccountingAuditEmitter` cuando `Formato != json`
- [x] T103 [P] [US5] Pruebas `TrialBalanceQueryHandlerTests` (cuadra; saldo = suma de líneas; excluye borradores; incluye reversiones netas; cierre excluido por defecto), `LedgerQueryHandlerTests` (nodos y saldos por nivel; alcance de sucursal), `IncomeStatementQueryHandlerTests` (rubros del grupo 2) en `AppTests/Accounting/Reports/`
- [x] T104 [US5] Crear `API/Endpoints/Reports/AccountingReportsEndpoints.cs` (`/api/reports/accounting/{vista}?format=`, un `MapGet` por vista con `EntregaDeInformes.EntregarAsync`; `Accounting.Reports.View` para `json`, `Accounting.Reports.Export` para el resto)
- [x] T105 [P] [US5] Crear `Shared/Services/Contabilidad/ContabilidadClient.Informes.cs` (JSON y descarga por formato; `TablaReporteDto` reutilizado desde nómina) y el componente `Shared/Components/Contabilidad/EncabezadoDeInforme.razor`
- [x] T106 [US5] Crear `Shared/Components/Contabilidad/LibroAuxiliar.razor` (`SfTreeGrid` con hijos por demanda desde `LedgerQuery`, columnas saldo inicial/débitos/créditos/final, exportar el nivel actual) y `Shared/Pages/Contabilidad/LibroAuxiliar.razor` (`/contabilidad/libro-auxiliar`: filtros combinables + árbol; abre el comprobante en `Comprobante.razor`)
- [x] T107 [US5] Crear `Shared/Pages/Contabilidad/Informes.razor` (`/contabilidad/informes`: selector de vista, filtros, tabla en pantalla y tres botones de exportación) y `Shared/Pages/Contabilidad/EstadosFinancieros.razor` (`/contabilidad/estados-financieros`: ESF, ERI, cambios en el patrimonio, flujo de efectivo, comparativo, incluir cierre); agregar las rutas a `Shared/Layout/NavMenu.razor`
- [x] T108 [US5] `IntegrationTests/Accounting/InformesTests.cs` (colección «Contabilidad e2e», helper `ContabilidadE2E` sobre `NominaE2E.PrepararAsync`; 2026-09-20): el balance cuadra tras digitar, contabilizar, reversar, cerrar y reabrir el período y la cuenta vuelve a su saldo; `ledger` desde las clases hasta las líneas siguiendo `_nodo`; los tres formatos con las mismas filas que json; `Accounting.Report.Exported` en `GET /api/audit/logs`; ReadOnly ve json y recibe 404 al exportar; un usuario asignado a la sucursal Norte no ve la Principal en ninguna de las 13 vistas (`[Theory]`); estado de cuenta del tercero y documentos pendientes con el cruce. Destapó dos defectos, corregidos: los emisores explícitos de auditoría escribían con el Id interno de la cooperativa (base `…_Audit_3` que la consola no lee) y `COR_Branches.TenantBranchPublicId` no lo escribía ningún comando (el alcance de sucursal no se podía configurar). **Pendiente**: `RendimientoDeInformesTests` (`RUN_PERF_TESTS=1`, un millón de líneas, SC-008)
...
- [x] T109 [US6] Crear `CloseFiscalYearCommand` (12 meses cerrados, ejercicio anterior cerrado, `ResultAccountId` definido; comprobante `Kind = Closing` tipo `CI` por `AccountingPoster` cancelando rubros de resultado contra la cuenta de resultado; `IReintentableAnteConcurrencia`) y `ReopenFiscalYearCommand` (motivo; `PrepareReversalAsync` del cierre) en `Application/Accounting/Periods/` — **Hecha 2026-09-21** (`Application/Accounting/Periods/FiscalYearCommands.cs`: CI por sucursal y centro de costo, fechado el 31/12 en período cerrado —el contrato lo admite sólo por ser `Closing`—, sin comprobante si no hay resultados; reabrir reversa en la misma fecha y clase, sólo el último cerrado)
- [x] T110 [P] [US6] Pruebas `CloseFiscalYearCommandHandlerTests` (meses abiertos; anterior abierto; sin cuenta de resultado; comprobante de cierre cuadrado y con fecha 31/12) y `ReopenFiscalYearCommandHandlerTests` en `AppTests/Accounting/Periods/` — **Hecha 2026-09-21** (`AppTests/Accounting/Periods/FiscalYearCommandsTests.cs`, 6 pruebas)
- [x] T111 [US6] Agregar `POST /years/{year}/close` y `/reopen` a `API/Endpoints/Accounting/PeriodsEndpoints.cs` (`Accounting.Periods.CloseYear`) y las acciones de ejercicio a `Shared/Pages/Contabilidad/Periodos.razor` y `ContabilidadClient.Periodos.cs` — **Hecha 2026-09-21** (rutas, `ContabilidadClient` y tarjeta «Cierre del ejercicio» en `Periodos.razor` con reabrir con motivo)
- [x] T112 [US6] Crear `E2E/Accounting/CierreDeEjercicioTests.cs` (cerrar con borrador → 422; cerrar los doce; cerrar el año; balance del 1 de enero siguiente sin resultados; estado de resultados sin/con cierre; reabrir con motivo y auditoría; cerrar 2027 con 2026 abierto → 422) — **Hecha 2026-09-21** (`IntegrationTests/Accounting/CierreDeEjercicioTests.cs` en cooperativa aislada con primer ejercicio 2025: incluye `IsClosing`, `NotLast`, balance del 1/1 siguiente, ERI con y sin cierre, auditoría de la reapertura)
...
- [x] T113 [US13] Crear `ImportOpeningBalancesCommand` (`ITabularFileReader`; plantilla `cuenta, tercero, tipoDocumento, numeroDocumento, centroCosto, sucursal, debito, credito, detalle`; errores por fila con `AccountLineRules`; crea borrador tipo `AP` `Kind = Opening`; `Accounting.Opening.AlreadyExists { openingDocumentPublicId }`, `.Invalid { errors[] }`) y `GetOpeningTemplateQuery` (tabla vacía con encabezados → xlsx por `ExportadorDeTablas`) en `Application/Accounting/Opening/` — **Hecha 2026-09-21** (`Application/Accounting/Opening/OpeningCommands.cs`; además `GetOpeningStatusQuery` para la pantalla)
- [x] T114 [US13] En `PostDocumentCommand`/`AccountingPoster` cerrar las reglas de `Opening` (fecha = primer período − 1, única no reversada, guarda `AccountingSetup.OpeningDocumentId`) y en `TrialBalanceQuery`/`LedgerQuery` tratar sus líneas como saldo inicial — **Hecha 2026-09-21** (`PostDocumentCommand`: única vigente y `OpeningDocumentId`; `ReverseDocumentCommand` la suelta; `SaveDraftDocumentCommand` admite AP con la fecha esperada; el poster reversa apertura y cierre en su fecha y clase; los informes de E2 ya trataban `Opening` como saldo inicial)
- [x] T115 [P] [US13] Pruebas `ImportOpeningBalancesCommandHandlerTests` (fila con agrupación; tercero inexistente; documento faltante; más de dos decimales; nada a medias; segunda apertura rechazada) en `AppTests/Accounting/Opening/` — **Hecha 2026-09-21** (`AppTests/Accounting/Opening/ImportOpeningBalancesCommandHandlerTests.cs`, 5 pruebas)
- [x] T116 [US13] Crear `API/Endpoints/Accounting/OpeningEndpoints.cs` (`/api/accounting/opening`: `GET /template.xlsx`, `POST /import` multipart; `Accounting.Opening.Manage`), `Shared/Services/Contabilidad/ContabilidadClient.Apertura.cs` y `Shared/Pages/Contabilidad/Apertura.razor` (`/contabilidad/apertura`: descargar plantilla, importar con errores por fila, abrir el borrador en `Comprobante.razor`, estado de la apertura; ruta en `NavMenu.razor`) — **Hecha 2026-09-21** (`OpeningEndpoints.cs` con `GET /`, plantilla por `PlantillaDeImportacion` —encabezados en la fila 1, no el formato de informe—, `Apertura.razor`, menú y tema del manual `saldos-de-apertura`)
- [x] T117 [US13] Crear `E2E/Accounting/AperturaTests.cs` (plantilla; importar con errores → 422 con filas; importar bien → borrador `AP`; con `RUN_PERF_TESTS=1`, 5 000 filas generadas en < 2 min (SC-014); contabilizar; balance del 1 de enero = totales del archivo al centavo; documentos pendientes del tercero; segunda apertura → 422; reversar y cargar otra) — **Hecha 2026-09-21** (`IntegrationTests/Accounting/AperturaTests.cs` en cooperativa aislada; 5.000 filas bajo `RUN_PERF_TESTS=1`)
...
- [x] T137 [P] [US9] Crear `SaveBudgetCommand` (líneas por cuenta de movimiento; `Accounting.Budget.AccountNotMovement`; crea versión con motivo si está aprobado), `ApproveBudgetCommand`, `CopyBudgetCommand` (año anterior + %; redondeo a pesos), `DistributeBudgetCommand` (igual/manual/porcentual; diferencia en el último mes), `GetBudgetQuery` en `Application/Accounting/Budgets/`
- [x] T138 [P] [US9] Crear `BudgetExecutionQuery` (presupuestado, ejecutado, variación y %, mes y acumulado, por cuenta y agregado por niveles; inicial y vigente; respeta `IUserBranchScope`) en `Application/Accounting/Reports/BudgetExecutionQuery.cs` y la vista `budget-execution` en `AccountingReportsEndpoints.cs`
- [x] T139 [P] [US9] Pruebas en `AppTests/Accounting/Budgets/` (agrupación rechazada; versión con motivo; copia +5 % redondeada; distribución igual con resto al último mes; ejecución y agregación)
- [x] T140 [US9] Crear `API/Endpoints/Accounting/BudgetsEndpoints.cs` (`Accounting.Budget.View|Manage`), `ContabilidadClient.Presupuesto.cs` y `Shared/Pages/Contabilidad/Presupuesto.razor` (`/contabilidad/presupuesto`: grilla cuenta × 12 meses, copiar, distribuir, aprobar, versiones, ejecución con profundización al libro auxiliar; ruta en `NavMenu.razor`)
- [x] T141 [US9] `IntegrationTests/Accounting/PresupuestoTests.cs` (2026-09-20): crear → 201 Draft; agrupación → 422 `AccountNotMovement`; aprobar; PUT sin motivo → 422 `ReasonRequired`; con motivo → versión 2 y `?version=1` intacta; distribuir `equal` 100 → 8 × 11 y 12 en diciembre; ejecución con un gasto contabilizado (mes, acumulado, %, inicial); copia a 2027 con +5 % en pesos
