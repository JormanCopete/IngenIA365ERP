...
- [ ] T095 Confirmar la rama con el usuario, merge a `develop`, CI verde, Argo en DEV/QA; verificar en las bases (`ACC_AccountCatalogs` con SOLIDARIO/COMERCIAL y conteos, `ACC_FinancialStatementItems`, `ACC_VoucherTypes`, 30 permisos, roles con `Accounting.*.View`)
- [ ] T096 QA manual `quickstart.md` §4 por rol (administrador, Operador, sólo lectura) y **validación de los catálogos por el contador** (`POST /catalogs/{code}/validate`); registrar resultado en `docs/operaciones/estado-y-pendientes.md` — bloqueante para producción
