...
- [ ] T028 Crear interfaz `ICentralIdentityProvider` en `src/Core/IngenIA365ERP.Application/Common/Abstractions/ICentralIdentityProvider.cs` con métodos: `ValidatePasswordAsync`, `CreateUserAsync`, `ChangePasswordAsync`, `EnrollMfaAsync`, `VerifyMfaAsync`, `DisableMfaAsync`, `ResetMfaAsync`, `FindByEmailAsync`, `IncrementAccessFailedAsync`, `ResetAccessFailedAsync`
- [ ] T029 [P] Crear interfaz `IEmailSender` en `src/Core/IngenIA365ERP.Application/Common/Abstractions/IEmailSender.cs` con método `SendAsync(EmailMessage)` y record `EmailMessage(To, Subject, HtmlBody, PlainTextBody)`. Reemplaza el `IEmailSender` actual ubicado en Storage (mover, no duplicar)
- [ ] T030 [P] Crear interfaz `IPwnedPasswordService` en `src/Core/IngenIA365ERP.Application/Common/Abstractions/IPwnedPasswordService.cs` con método `IsPwnedAsync(string password, CancellationToken) → PwnedPasswordResult { IsPwned, ServiceAvailable }`
- [ ] T031 [P] Crear interfaz `ITenantMembershipReader` en `src/Core/IngenIA365ERP.Application/Common/Abstractions/ITenantMembershipReader.cs` con métodos `GetActiveMembershipsAsync(Guid centralUserId)`, `IsMemberOfTenantAsync(Guid centralUserId, Guid tenantId)`, `InvalidateCacheAsync(Guid centralUserId)`
- [ ] T032 [P] Crear constantes de `AuditEventTypes` para los 12 nuevos eventos (`CentralUser.Login.{Success,Failed,Locked}`, `CentralUser.Mfa.{Success,Failed,ResetByMaster}`, `Invitation.{Issued,Accepted,Expired,Revoked,Superseded}`, `Membership.{Activated,Suspended,Revoked,PromotedToAdmin,DemotedFromAdmin,LastAdminProtected}`, `Session.{TenantSelected,TenantSwitched}`, `TenantMfaPolicy.{Activated,Deactivated}`, `Profile.DefaultTenantChanged`, `Tenant.Created`) en `src/Core/IngenIA365ERP.Application/Common/Audit/AuditEventTypes.cs`
...
- [ ] T035 [P] Crear `src/Infrastructure/IngenIA365ERP.Identity/Hashers/BcryptPasswordHasher.cs` implementando `IPasswordHasher<CentralUserIdentity>` con BCrypt.Net-Next cost 11 (soporta detección de hashes con cost inferior + rehash on verify)
- [ ] T036 [P] Crear `src/Infrastructure/IngenIA365ERP.Identity/Services/PwnedPasswordService.cs` con `HttpClient` named `pwned`, k-anonymity (SHA-1 → primeros 5 chars → GET `https://api.pwnedpasswords.com/range/{prefix}` → comparar sufijos), timeout 2s, fail-open con Serilog Warning `Pwned.UnavailableFallback`
- [ ] T037 [P] Crear `src/Infrastructure/IngenIA365ERP.Identity/Services/CentralJwtIssuer.cs` que emita JWT RS256 con claims `sub`, `email`, `is_global_master_admin`, `active_tenant_id` (opcional), `tenant_admin` (opcional), `mfa_verified`, `purpose` (`full` | `mfa-verify` | `mfa-enroll` | `tenant-select` | `password-reset`), `iat`, `exp`, `nbf`, `jti`. Métodos: `IssueAccessToken(centralUser, activeTenant?, isMfaVerified, purpose=full)`, `IssueChallengeToken(centralUser, purpose, ttlMinutes=5)`, `IssueRefreshToken`, `RotateRefreshToken`. Solo los tokens con `purpose=full` se aceptan en endpoints de negocio; el resto son scope-limited.
- [ ] T038 [P] Crear `src/Infrastructure/IngenIA365ERP.Identity/Services/TenantMembershipReader.cs` implementando `ITenantMembershipReader` con cache Redis (delegando a `TenantMembershipCache`) + fallback a `AdminDbContext`
- [ ] T039 [P] Crear `src/Infrastructure/IngenIA365ERP.Caching/Services/TenantMembershipCache.cs` (key `memberships:{centralUserId}`, TTL 60s, JSON serializer, suscriptor a canal Redis pub/sub `membership-changed`)
- [ ] T039a [P] Crear abstracción `IMembershipChangedNotifier` en `src/Core/IngenIA365ERP.Application/Common/Abstractions/IMembershipChangedNotifier.cs` con método `PublishAsync(Guid centralUserId)`, y su implementación `RedisMembershipChangedNotifier` en `src/Infrastructure/IngenIA365ERP.Caching/Services/RedisMembershipChangedNotifier.cs` que publica en el canal `membership-changed` (consumido por T039 en cada instancia). Inyectado en todos los handlers que muten membresías o políticas (T093-T099, T079b, T079c, T097, T099, T102).
- [ ] T040 [P] Crear `src/Infrastructure/IngenIA365ERP.Caching/Services/LoginAttemptCounter.cs` con escalado progresivo según `research.md > D-11` (incrementa contador en Redis, devuelve `LockoutVerdict { ShouldLock, LockSeconds }`)
- [ ] T041 Crear `src/Infrastructure/IngenIA365ERP.Identity/Providers/AspNetCoreIdentityProvider.cs` implementando `ICentralIdentityProvider` usando `UserManager<CentralUserIdentity>` + `SignInManager<CentralUserIdentity>`. Inyecta `IPwnedPasswordService` en `CreateUserAsync` y `ChangePasswordAsync`. (depende de T028, T030, T033, T034, T035)
...
- [ ] T045 [P] Crear `src/Presentation/IngenIA365ERP.API/Middleware/CentralIdentityChallengeMiddleware.cs` que traduzca `401` no autenticado a respuesta JSON `{ "errorCode": "Identity.Unauthenticated" }` consistente con el `ErrorEnvelopeFilter`
- [ ] T046 [P] Crear filtro `src/Presentation/IngenIA365ERP.API/Filters/RequireTenantAdminAttribute.cs` que verifique claim `tenant_admin == true`, `active_tenant_id` igual al `{tenantPublicId}` de la ruta, **y `purpose == full`** (rechaza tokens de challenge)
- [ ] T047 [P] Crear filtro `src/Presentation/IngenIA365ERP.API/Filters/RequireMasterAdminAttribute.cs` que verifique claim `is_global_master_admin == true` **y `purpose == full`**
- [ ] T047a [P] Crear filtro `src/Presentation/IngenIA365ERP.API/Filters/RequirePurposeAttribute.cs` con parámetro `params string[] allowedPurposes` para endpoints scoped (login challenges). Uso típico: `[RequirePurpose("mfa-enroll", "full")]` sobre `/api/profile/mfa/enroll` y `/confirm`, `[RequirePurpose("mfa-verify")]` sobre `/api/auth/mfa/verify`, `[RequirePurpose("tenant-select", "full")]` sobre `/api/sessions/select-tenant`
...
- [ ] T049 [P] Crear `tests/IngenIA365ERP.Architecture.Tests/CentralIdentityArchTests.cs`: (a) `ICentralIdentityProvider` solo accesible vía Application; (b) ningún tipo en Application referencia `UserManager`/`SignInManager`; (c) ningún tipo fuera de `IngenIA365ERP.Identity` referencia `Microsoft.AspNetCore.Identity.*`
- [ ] T050 [P] Crear `tests/IngenIA365ERP.Architecture.Tests/AdminDbContextArchTests.cs`: (a) `AdminDbContext` NEVER es inyectado en handlers de Application que toquen entidades de tenant; (b) ningún DTO público expone `int Id` de las entidades non-Identity (`TenantMembership`, `Invitation`, `TenantMfaPolicy`); (c) entidades en `Admin/` heredan `AuditableEntity` salvo `CentralUserLoginAttempt`
