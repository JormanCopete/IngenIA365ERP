...
$ddlFiles = @(
    "tools\scripts\IngenIA365ERP_Schema_01_COR.sql",
    "tools\scripts\IngenIA365ERP_Schema_02_ACC.sql",
    "tools\scripts\IngenIA365ERP_Schema_03_LND.sql"
)
...
foreach ($file in $ddlFiles) {
    ...
    if (-not (Test-Path $fullPath)) {
        Write-Error "No se encontr� el archivo: $fullPath"
        exit 1
    }

    Write-Host "Ejecutando $file..."

    sqlcmd -S $Server -U $User -P $Password -i $fullPath
