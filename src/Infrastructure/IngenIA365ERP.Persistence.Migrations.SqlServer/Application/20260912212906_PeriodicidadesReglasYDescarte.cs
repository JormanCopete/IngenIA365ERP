    ...
    public partial class PeriodicidadesReglasYDescarte : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.Sql("""
                UPDATE p
                SET p.ImputationYear = YEAR(p.StartDate),
                    p.ImputationMonth = MONTH(p.StartDate),
                    p.SubPeriodNumber = CASE pl.Periodicity
                        WHEN 15 THEN CASE WHEN DAY(p.StartDate) <= 15 THEN 1 ELSE 2 END
                        ELSE 1 END
                FROM dbo.PAY_PayPeriods p
                INNER JOIN dbo.PAY_PayrollPlans pl ON pl.Id = p.PayrollPlanId
                WHERE p.ImputationYear = 0;
                """);
        }
