...
namespace IngenIA365ERP.Application.Core.Positions.Commands.UpdatePosition;

public record UpdatePositionCommand : IRequest<Result>
{
    ...
    : IRequestHandler<UpdatePositionCommand, Result>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var repetido = await context.Positions.AsNoTracking()
                .FirstOrDefaultAsync(e => e.LegacyCode == codigo && e.Id != entity.Id && !e.IsDeleted, cancellationToken);
            if (repetido is not null)
                return Result.Failure(CodigoDeCatalogo.Duplicado("un cargo", codigo, repetido.Name));
        }
