...
/// <summary>
/// Los campos editables de una versión de concepto (contracts/api.md §6). Feature 010 (D-37):
    ...
    : IRequestHandler<ReviseConceptDefinitionCommand, Result<Guid>>
{
    public async Task<Result<Guid>> Handle(ReviseConceptDefinitionCommand request, CancellationToken ct)
    {
        ...
        // Lo que el cliente no manda se hereda de la versión vigente (D-37): la base de vacaciones y la ruta
