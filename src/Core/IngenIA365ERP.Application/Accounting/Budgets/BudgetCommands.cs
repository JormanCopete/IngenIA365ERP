...
public sealed class CreateBudgetCommandHandler(IApplicationDbContext db, IDateTimeService clock, ICurrentUserService user, AccountingAuditEmitter audit)
    : IRequestHandler<CreateBudgetCommand, Result<BudgetDto>>
{
    public async Task<Result<BudgetDto>> Handle(CreateBudgetCommand request, CancellationToken ct)
    {
        ...
        var lineas = await LineasDePresupuesto.ResolverAsync(db, request.Lines, ahora, quien, ct);
        ...
        return await ArmadoDePresupuesto.ArmarAsync(db, request.Year, null, ct);
    }
}
...
public sealed class UpdateBudgetCommandHandler(IApplicationDbContext db, IDateTimeService clock, ICurrentUserService user, AccountingAuditEmitter audit)
    : IRequestHandler<UpdateBudgetCommand, Result<BudgetDto>>
{
    public async Task<Result<BudgetDto>> Handle(UpdateBudgetCommand request, CancellationToken ct)
    {
        ...
        var r = EscrituraDePresupuesto.Aplicar(db, ejercicio, vigente, conservar: [], retirar: vigente.Lineas, agregar: lineas.Value,
...
public sealed class ApproveBudgetCommandHandler(IApplicationDbContext db, IDateTimeService clock, ICurrentUserService user, AccountingAuditEmitter audit)
...
public sealed class CopyBudgetCommandValidator : AbstractValidator<CopyBudgetCommand>
{
    public CopyBudgetCommandValidator()
    {
        ...
        RuleFor(x => x.AdjustPercent).GreaterThan(-100m);
    }
}

public sealed class CopyBudgetCommandHandler(IApplicationDbContext db, IDateTimeService clock, ICurrentUserService user, AccountingAuditEmitter audit)
    : IRequestHandler<CopyBudgetCommand, Result<BudgetDto>>
{
    public async Task<Result<BudgetDto>> Handle(CopyBudgetCommand request, CancellationToken ct)
    {
        ...
        {
            var r = EscrituraDePresupuesto.Aplicar(db, ejercicio, vigente, conservar: [], retirar: vigente.Lineas, agregar: copiadas, null, permitirVersionar: false, ahora, quien);
            if (r.IsFailure) return Result.Failure<BudgetDto>(r.Error);
            destino = r.Value;
        }
        await db.SaveChangesAsync(ct);
...
public sealed class DistributeBudgetCommandHandler(IApplicationDbContext db, IDateTimeService clock, ICurrentUserService user, AccountingAuditEmitter audit)
    : IRequestHandler<DistributeBudgetCommand, Result<BudgetDto>>
{
    public async Task<Result<BudgetDto>> Handle(DistributeBudgetCommand request, CancellationToken ct)
    {
        ...
        var ahora = clock.UtcNow;
        var quien = user.UserName ?? "system";
        var lineas = await LineasDePresupuesto.ResolverAsync(db, [new BudgetLineInput(request.AccountPublicId, request.BranchPublicId, request.CostCenterPublicId, cuotas.Value)], ahora, quien, ct);
        if (lineas.IsFailure) return Result.Failure<BudgetDto>(lineas.Error);
...
public static class LineasDePresupuesto
{
    ...
    /// centro existentes, doce valores no negativos por fila y ninguna (cuenta, sucursal, centro)
    /// repetida —se comprueba aquí porque en PostgreSQL el índice único con NULL no lo frena—. Los
    /// meses en cero no se guardan: el DTO los reconstruye.
    /// </summary>
    public static async Task<Result<List<BudgetLine>>> ResolverAsync(IApplicationDbContext db, IReadOnlyList<BudgetLineInput> inputs, DateTime ahora, string quien, CancellationToken ct)
    {
        ...
        var sucursales = sucursalIds.Count == 0
            ? []
            : await db.Branches.AsNoTracking().Where(b => sucursalIds.Contains(b.PublicId) && !b.IsDeleted).ToDictionaryAsync(b => b.PublicId, b => b.Id, ct);
        ...
        foreach (var input in inputs)
        {
            ...
            {
                if (!sucursales.TryGetValue(s, out var id)) return Result.Failure<List<BudgetLine>>(BudgetErrors.BranchNotFound);
                sucursalId = id;
            }
            int? centroId = null;
