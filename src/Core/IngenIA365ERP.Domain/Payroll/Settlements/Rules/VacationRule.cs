...
/// la ordinaria descuenta— si la política paga anticipado (D-01, D-29); en una compensación, los
...
public static class VacationRule
{
        ...
        switch (movimiento.Kind)
        {
            case VacationMovementKind.Enjoyment:
            {
                ...
                exp.Note("Disfrute", $"{Fechas(movimiento).TrimStart()}: {Fmt.Num(movimiento.BusinessDays)} días hábiles, {movimiento.CalendarDays} calendario " +
                                     $"(D-01 y D-29: se pagan los {diasPagados} días del descanso por el calendario comercial de la nómina, los mismos que la ordinaria descuenta del salario).");
    ...
    /// <summary>
    /// Los días del descanso que la liquidación paga (D-29): los del calendario comercial de la nómina
